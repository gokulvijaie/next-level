/**
 * POST /api/reviews — submits a review to Drupal server-to-server.
 * The shared secret never reaches the browser; the client IP is forwarded
 * for Drupal's flood control.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/storefront/reviews`, {
      method: "POST",
